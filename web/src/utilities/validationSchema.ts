...
export const regSchema = yup.object({
    ...
    .test('has-no-space', 'Name must not contain spaces', value => isNoSpaces(value)),
    email: yup.string().required().email("Invalid email")
    .test('has-no-space', 'Email must not contain spaces', value => isNoSpaces(value)),
    password: yup.string().required()
    .min(8, "Password must be at least 8 characters").max(20, "Password must be at most 20 characters")
    .test('has-no-space', 'Password must not contain spaces', value => isNoSpaces(value))
